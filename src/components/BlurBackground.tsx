...
const BlurBackground: React.FC<BlurBackgroundProps> = ({ className }) => {
  return (
    <div className={`fixed inset-0 z-[-1] bg-noise opacity-30 pointer-events-none ${className}`}>
      <div className="absolute top-20 left-20 w-96 h-96 bg-evoya-blue/20 rounded-full blur-3xl -z-10 animate-pulse-slow" />
      <div className="absolute top-1/3 right-1/4 w-80 h-80 bg-evoya-green/20 rounded-full blur-3xl -z-10 animate-pulse-slow" style={{ animationDelay: '1s' }} />
